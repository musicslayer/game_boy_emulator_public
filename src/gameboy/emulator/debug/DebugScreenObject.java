...
public class DebugScreenObject extends Screen {
    public int[] colorIndexMap0 = new int[4];
    public int[] colorIndexMap1 = new int[4];

    public int[][] imageData;
    
    public DebugScreenObject(AddressMap addressMap) {
        super(addressMap);
    ...
    public int getWidth() {
        // The width in pixels - 20 tile blocks - 8 pixels / tile
        return 160;
    }

    public int getHeight() {
        // The height in pixels - 18 tile blocks - 8 pixels / tile
        return 144;
    }

    public void drawScreen() {
        // Because objects don't fill entire display, make sure unused space is reset to white every time.
        resetImageData();

        boolean isLargeMode = addressMap.loadBit(AddressMap.ADDRESS_LCDC, 2) == 1;
        for(int[] objectAttribute : getObjectAttributes()) {
            ...
            int tileX = objectAttribute[1] - 8;
            int tileY = objectAttribute[0] - 16;
